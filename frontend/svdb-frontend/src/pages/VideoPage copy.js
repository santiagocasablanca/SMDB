...
const VideoPage = () => {
  ...
  const { intToStringBigNumber, parseDate, parseDateToFromNow, parseDuration } = useFormatter();
                    ...
                    renderItem={(creator, index) => (
                      <List.Item key={creator.id} onClick={() => goToCreator(creator.id)} className="showPointer">
                        <List.Item.Meta onMouseEnter={(e) => {
                          ...
                          title={<><Text>{creator.name}</Text> <Text italic type="secondary" style={{ color: 'gray' }}> as {creator.video_creator.role}</Text></>}
