...
import { Button, Card, Col, Image, Input, List, Row, Avatar, Space, Typography } from 'antd';
...
const ChannelsPage = () => {
  ...
  const fixCoverUrl = (url) => {
    return url ? url.includes('=w2560-fcrop64=') ? url : url + '=w2560-fcrop64=1,00005a57ffffa5a8-k-c0xffffffff-no-nd-rj' : null;
  }
    ...
    <div className="channelsBodyContainer">
      ...
      <List
        ...
        renderItem={(item, index) => (
          <List.Item>
            <Card
              style={{ width: '100%', maxWidth: '450px', border: 'none', backgroundColor: 'transparent' }}
              bodyStyle={{ padding: 0 }}

              cover={
                <Image alt={item.name}
                  style={{ height: '160px', objectFit: 'cover' }}
                  src={fixCoverUrl(item?.banner_url)}
              ...
              <div style={{ padding: '5px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', color: 'black', marginBottom: '5px', alignItems: 'flex-start' }}>
                  <div style={{display: 'flex'}}>
                    <Avatar src={item.logo_url} onClick={() => handleClickChannel(item.channel_id)} style={{
                      backgroundColor: '#f56a00', marginRight: '5px', cursor: 'pointer'
                    }} size="large" />
                    <div>
                      <Text style={{ color: 'black', cursor: 'pointer', fontSize: '15px' }} strong onClick={() => handleClickChannel(item.channel_id)}>{item.title}</Text>
              ...
              {/* <Title style={{color: 'black', textAlign: 'center', textTransform: 'uppercase'}} level={5}>{item.title}</Title>  */}
            </Card>
          </List.Item>
        )}
      />
