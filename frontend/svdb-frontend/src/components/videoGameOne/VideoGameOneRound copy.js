...
const VideoGameOneRound = ({ _round, totalPoints, remainingLives, handleNextVideo, handlePrevVideo, handleGuessCB }) => {
  ...
  return (<>
    ...
    {isFetched && round ? <div style={{ backgroundColor: "rgba(48, 48, 48, 0.8)",  height: "100%" }}>
      ...
      <div>
        <div className="bttm-container">
          <Row style={{ paddingRight: '14px', paddingLeft: '14px' }}>
            <Col xs={24} sm={24} md={24} lg={12} xl={12}>
              <Card style={{ backgroundColor: "rgba(48, 48, 48, 0.8)" }} bodyStyle={{ padding: '10px 15px', height: '100px' }} >
        ...
        <div className="mobile-playing-card">
          ...
          <table key="gameinfo">
            <tbody>
              ...
              <tr>
                ...
                <td style={{ textAlign: 'right' }}>{round?.guesses.map(el => { return <Text strong>{channels.find(channel => channel.channel_id === el)?.title} </Text> })}</td>
              </tr>
              <tr>
          ...
          <Row>
            <Col span={12}>
              ...
              <Text strong>Helps</Text>
              <br></br>
              <Space.Compact direction="vertical">
                <Tooltip title="Unblur Thumbnail (-150p)"><Button disabled={round?.used_thumbnail_help} onClick={() => handleThumbnailHelpClick()}>Unblur Thumbnail</Button></Tooltip>
                <Tooltip title="See Stats (-200p)"><Button disabled={round?.used_stats_help} onClick={() => handleStatsHelpClick()}>See Stats</Button></Tooltip>
                <Tooltip title="See Title (-150p)"><Button disabled={round?.show_title} onClick={() => handleSeeTitleClick()}>See Title</Button></Tooltip>
              </Space.Compact>
            </Col>
        ...
        <Card size="small" className="playing-card" bodyStyle={{ padding: '10px 15px', width: '345px', height: '150px', overflowY: 'auto' }}
          actions={[
            <Tooltip title="Previous"><Button disabled={round?.round_number === 1} onClick={() => prevVideo()}><ArrowLeftOutlined /></Button></Tooltip>,
            <Tooltip title="Next"><Button disabled={!round?.guessed} onClick={() => nextVideo()}><ArrowRightOutlined /></Button></Tooltip>,
            <Tooltip title="Guess"><GuessBttn handleGuess={(titleGuess, channelGuess) => handleGuess(titleGuess, channelGuess)} /></Tooltip>
